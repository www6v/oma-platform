...
#
# Usage:
#   ./scripts/e2e/smoke-runtime-e2e.sh
#   RUNTIME_ACP=0 ./scripts/e2e/smoke-runtime-e2e.sh   # skip Claude Code turn
set -euo pipefail
...
phase_acp_turn() {
  ...
  if [[ ! -d "${ROOT_DIR}/scripts/e2e/node_modules/ws" ]]; then
    log "installing ws in scripts/e2e/ for probe..."
    (cd "${ROOT_DIR}/scripts/e2e" && npm install ws@8 --no-save 2>/dev/null || npm install ws@8)
  fi
...
export NODE_PATH="${ROOT_DIR}/scripts/e2e/node_modules${NODE_PATH:+:${NODE_PATH}}"
