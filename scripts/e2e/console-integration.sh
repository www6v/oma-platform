...
if [[ -x "$(command -v node)" ]] && [[ -f "${ROOT_DIR}/scripts/e2e/console-ui-check.mjs" ]]; then
  if node -e "import('playwright')" >/dev/null 2>&1; then
    echo "==> Console UI smoke (Playwright headless)"
    node "${ROOT_DIR}/scripts/e2e/console-ui-check.mjs"
    if [[ -f "${ROOT_DIR}/scripts/e2e/console-dogfood.mjs" ]]; then
      echo "==> Console UI dogfood (create agent + session + message)"
      node "${ROOT_DIR}/scripts/e2e/console-dogfood.mjs"
    fi
  else
    echo "==> skip UI check (npm install playwright in scripts/e2e/ && npx playwright install chromium)"
  fi
fi
