...
受保护路由支持请求头 `x-api-key: $OMA_API_KEY` 或 better-auth Cookie 会话。仅本地开发可设 `AUTH_DISABLED=1`（勿用于生产）。
...
浏览器打开 http://localhost:8787 — `./start-console.sh` 会启动 better-auth 侧车并代理 `/auth/*`，可用邮箱密码注册登录。
...
**生产环境：** 配置稳定的 `BETTER_AUTH_SECRET`，运行 auth 侧车并保持 `AUTH_DISABLED=0`。API 客户端仍可使用 `x-api-key`。
