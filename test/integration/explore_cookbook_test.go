...
// TestExploreCookbookUnfamiliarCodebase is the Go-server parity probe for
// CMA_explore_unfamiliar_codebase: zip mount + mid-session resources.add/delete.
func TestExploreCookbookUnfamiliarCodebase(t *testing.T) {
	sim := &demo.ExploreSimulatingClient{}
	handler := newExploreCookbookRouter(t, sim)
	integrationtest.RunExploreUnfamiliarCodebaseFlow(t, handler, sim)
}

func newExploreCookbookRouter(
	t *testing.T,
