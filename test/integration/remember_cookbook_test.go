...
// TestRememberCookbookPreferences is the Go-server parity probe for
// CMA_remember_user_preferences: memory_store cross-session recall.
func TestRememberCookbookPreferences(t *testing.T) {
	sim := &demo.RememberSimulatingClient{}
	handler := newRememberCookbookRouter(t, sim)
	integrationtest.RunRememberPreferencesCookbookFlow(t, handler, sim)
}

func newRememberCookbookRouter(
	t *testing.T,
