...
// TestCoordinateCookbookSpecialistTeam is the Go-server parity probe for
// CMA_coordinate_specialist_team: multiagent coordinator + 3 specialists.
func TestCoordinateCookbookSpecialistTeam(t *testing.T) {
	sim := &demo.CoordinateSimulatingClient{}
	handler := newCoordinateCookbookRouter(t, sim)
	integrationtest.RunCoordinateSpecialistTeamFlow(t, handler, sim)
}

func newCoordinateCookbookRouter(
	t *testing.T,
