...
create_agent() {
  ...
  api /v1/agents -X POST -d "{\"name\":\"$name\",\"model\":\"$MODEL\",\"system\":\"$system\",\"tools\":[{\"type\":\"agent_toolset_20260401\"}]}" | jq -r .id
}
...
send_and_collect() {
  ...
  local f=$(mktemp)
  # Start SSE listener
  curl -sS -N "$BASE/v1/sessions/$sid/events/stream" \
